...
class InputHandler : InputAxisControllerBase<InputHandler.Reader>
{
        ...
        else
        {
            ...
            PlayerInput.onActionTriggered += (value) =>
            {
                ...
                for (var i = 0; i < Controllers.Count; i++)
                    Controllers[i].Input.ProcessInput(value.action, GetSens(), PlayerInput.currentControlScheme);
            };
        }
    }
    public Vector2 GetSens()
    {
        return new Vector2(HorizontalSensitivity,-VerticalSensitivity);
        }
    ...
    public class Reader : IInputAxisReader
    {
        ...
        public void ProcessInput(InputAction action, Vector2 Sensitivity, string currentControl)
        {
            // If it's my action then cache the new value
            if (Input != null && Input.action.id == action.id)
            {
                if (action.expectedControlType == "Vector2" && currentControl == "Gamepad") m_Value = action.ReadValue<Vector2>() * Sensitivity * 10;
                else if (action.expectedControlType == "Vector2")
                    m_Value = action.ReadValue<Vector2>() * Sensitivity ;
